...
## 14. Reactome pathway analysis

`src/R/reactome_analysis.R` submits to Reactome's Analysis Service REST API
(`POST reactome.org/AnalysisService/identifiers/`) rather than the web UI or
the `ReactomePA` R package — chosen because it's the only option that both
(a) stays fully scripted/local and (b) can hand off to the interactive
Reactome Pathway Browser afterward. `ReactomePA` computes entirely against a
local Bioconductor annotation database and never talks to Reactome's
service, so there's no token and no browser hand-off; it also requires
Entrez Gene IDs (UniProt -> Entrez conversion needed), whereas the REST API
takes UniProt accessions directly. *(Appendix F3: R HTTP client)*

Two modes, auto-selected by the input file: a plain ID list (one accession
per line) runs over-representation analysis (ORA); a CSV with an ID column
and a log2FC-like column runs Reactome's expression analysis instead
(pathway diagrams coloured by the supplied values) — matching the paper's
own method (§5, "Reactome graphs and pathway analysis"). The expression
mode is implemented and ready, but not yet run for real — needs real
per-condition log2FC values, i.e. §4.

Every submission produces two outputs in `results/PXD025280_20260816/`:
- **`reactome_pathways.csv`** — the durable record (pathway ID, name,
  entities found/total, p-value, FDR), independent of anything expiring.
- **`reactome_token.json`** — the analysis token plus ready-to-use Pathway
  Browser URLs (`.../PathwayBrowser/#DTAB=AN&ANALYSIS={token}` for the
  overview, or `#{pathway_stId}&DTAB=AN&ANALYSIS={token}` for a specific
  diagram) for interactive visual exploration. **Only valid for ~7 days, or
  until Reactome's next quarterly data update — whichever is sooner** —
  after which the token has to be regenerated by resubmitting.

Run in plain-list (ORA) mode against §11's real 1,477-protein identified
list: **1,886 pathways, 421 with FDR < 0.05**. Translation/ribosome-related
pathways dominate the top of the list — worth being explicit that this
reflects which pathways are well-represented among the proteins this
experiment successfully identified and quantified (ribosomal proteins are
